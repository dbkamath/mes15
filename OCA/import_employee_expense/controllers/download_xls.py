...
class Download_xls(http.Controller):
    ...
    def download_document(self,model,id, **kw):
        ...
        if res.sample_option == 'xls':
            invoice_xls = request.env['ir.attachment'].search([('name','=','HR_Expense.xls')])
            filecontent = invoice_xls.datas
            filename = 'HR_Expense.xls'
            filecontent = base64.b64decode(filecontent)

        elif res.sample_option == 'csv':
            invoice_xls = request.env['ir.attachment'].search([('name','=','HR_Expense.csv')])
            filecontent = invoice_xls.datas
            filename = 'HR_Expense.csv'
            filecontent = base64.b64decode(filecontent)
